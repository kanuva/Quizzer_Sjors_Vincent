...
module.exports.listen = function (server) {
    ...
    io.on('connection', function (socket) {
        ...
        socket.on('sentQuestion', function (data) {
            ...
            Game.findOne({'master': data.masterId}).exec(function (error, room) {
                if (!error) {
                    ...
                    io.to(room.scoreboard).emit('scoreboard_refresh');
                    console.log("ik zou iets moeten sturen nu naar de scoreoboard: " + room.scoreboard);
                }
            });
        ...
        socket.on('team_create', function (data) {
            var exists = false;
            console.log("iemand wilt joinen namelijk: " + data.team);
